...
| 05 group-mode-selection | #113 | shared mode registry (generalize `web/src/pages/soloModes.ts`), host mode picker on `Lobby.tsx`, `GroupRound.tsx` (resolve `round.mode` -> surfaces/config), `GameHub.cs` (`StartRound` `mode` param + per-mode template pick; populate `RoundStartedDto.Mode`), `useGameHub.ts` | gp/01-04, single-player/02 (registry), game-modes/03-06, child-safety/02 | - | 5 | medium |
