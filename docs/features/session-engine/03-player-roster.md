...
## Acceptance Criteria
- [ ] AC-01: Given I am in a room, then I see a 3-column grid of player tiles;
...
- [ ] AC-02: Given a new player joins, then their tile fills an empty slot with a
      ...
      Gotchas.
- [ ] AC-03: Given a player slot is not yet filled, then it renders as a dashed
      circle with 3 pulsing dots and "waiting..." text; the border color pulses
      purple.
- [ ] AC-04: Given a player leaves (closes the app or navigates away), then the
      roster reflects their departure within a short, reasonable window (slot
      reverts to empty).
- [ ] AC-05: Given I am the host, then I can see the current player count and the
...
- [ ] AC-06: Given the roster is displayed, then every name shown has passed the
