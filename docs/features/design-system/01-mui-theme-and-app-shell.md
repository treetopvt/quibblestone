...
## Acceptance Criteria
- [ ] AC-01: Given the app loads, then the background renders the parchment
      gradient (`#F8F1E2` to `#F6EEDD` to `#F0E6D0`, top to bottom) from the
      MUI theme palette, with no hardcoded color in any component.
- [ ] AC-02: Given any heading or button text, then it renders in Fredoka
...
- [ ] AC-03: Given a screen uses the shared `<AppBar>` component, then it
...
- [ ] AC-04: Given a screen uses the gold primary CTA button, then it renders
...
- [ ] AC-05: Given a screen uses the outlined-purple secondary button, then it
...
- [ ] AC-06: Given the bottom action bar pattern (pinned actions), then scrollable
...
- [ ] AC-07: Given the app runs on a 390px-wide portrait viewport, then content
...
- [ ] AC-08: Given the hero mascot asset (the full-size, posed stone-guardian
