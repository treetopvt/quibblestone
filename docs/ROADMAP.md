...
| Load / stress test | [`docs/load-testing/findings.md`](./load-testing/findings.md) + the `/load` harness | Architecture holds well past alpha scale (1,200 conns / 200 rooms / 100% completion, 0 errors locally). Real ceiling is single in-process instance / no Azure SignalR backplane (F1). Surfaced W2 (no player cap -> a 1-per-room join race + O(N^2) roster fan-out in large rooms) - **now fixed on this branch** (the 6-player cap). Run the harness vs UAT for the true B1 knee |
...
| W2 | Warn | Lobby says "n of 6" but the server never enforces a cap (a 7th joiner shows "7 of 6"; on F1 they cannot connect at all) | enforce the cap in `JoinRoom` with a friendly "room's full" | **Fix on branch** `claude/ga4-analytics-stress-test-7tjpar` - `Room.AddPlayer` caps at `Room.MaxPlayers`=6 (host incl.), atomic under the room lock; `JoinRoom` returns the friendly full message. Surfaced + verified by the load test (F2) |
