...
| Billing-mode toggle relocation | `billing-entitlements/07` follow-up - **DONE** via `sysadmin-console/04` (#205, 2026-07-09) | moved `/admin/billing-mode` out of the kid bundle into the operator console behind the real Operator policy; the interim `IOperatorGate` shared-secret scheme is deleted |
...
| The admin platform (ADR 0003) | `docs/adr/0003-admin-platform-and-family-accounts.md` + stories in `accounts-identity/05-09`, `keepsake-vault/01-04` (new), `control-plane/01-03` (new), `sysadmin-console/04-07`, `billing-entitlements/08`, `platform-devops/08` (specced 2026-07-08; **Waves 1-4 shipped 2026-07-09 - COMPLETE**) | the future-proof admin capability: stable AccountId spine + free family accounts (kid seat presets, family device link), the server-side keepsake vault (fixes "where are my saved stories"), the runtime control plane (settings + system-scope flags), and the reshaped operator console (one auth, jobs shell, action log, support lookup). **All four waves are merged and green - the admin platform is COMPLETE.** Wave 1 (foundation): AccountId spine (#201), keepsake vault auto-save (#204), runtime settings service (#206), one-console-one-auth (#205), durable key ring (#207). Wave 2 (capabilities + billing): Decision F at CreateRoom (#225), free family account (#220), gallery-over-vault (#222), system-flag scopes (#223), jobs shell + scoped authz (#221), grant metadata + Stripe resync (#226). Wave 3 (family accounts + recovery + the action log): kid seat presets (#236), family device link + teen-plus gate (#241), vault claim/recovery (#240), vault soft-delete/restore (#237), knob migration (#239), operator action log (#238). Wave 4 (the support surface): support lookup + verbs behind the structural cross-plane firewall (#245). Layers 0-3 are complete, so Stripe live is unblocked (a deliberate, separate go-live); the friends-and-family alpha continues on beta. The second environment (ADR Decision 4) is already delivered by the shipped `platform-devops/07` QA lane. Cross-feature build order lives in the ADR |
...
### 2.5 The admin platform (ADR 0003 - specced 2026-07-08, Waves 1-4 shipped 2026-07-09 - COMPLETE, runs alongside the beta)
- **Shipped (Wave 4, the support surface - 2026-07-09) - ADR 0003 ADMIN PLATFORM COMPLETE:** the
  final story, `sysadmin-console/07` (support lookup + verbs, #245, merged on owner sign-off - a
  flagged cross-plane-firewall seam), is merged to `main` and green (API 932 + web 520 tests). An
  operator finds an account by email/`AccountId` ONLY and sees an account-plane picture (grants,
  subscription state, vault/tale + linked-device COUNTS) with five verbs (resend link, extend a
  public tale's TTL, restore a keepsake, comp/extend via story 02, resync a subscription), each
  writing one log-before-act action-log row. The AC-08 firewall is STRUCTURAL (reflection +
  source-scan tests): the controller injects only narrow count/enum/instant contracts, never a
  byline-bearing store, so a slug/claim-code can never resolve to an account and no byline is
  projectable. Dependency-tolerant follow-ups (not blockers): keepsake-vault to expose an
  account -> vault COUNT projection (the vault count reads "not available yet" via a sentinel until
  then); a story-03 follow-up to give the moderation-takedown restore stronger friction than this
  courtesy restore; a concurrency regression test for the resync-debounce CAS. With all four waves
  in, **Layers 0-3 are complete and Stripe live is unblocked** (a deliberate, separate go-live); the
  friends-and-family alpha continues on beta.
- **Shipped (Wave 3, family accounts + recovery + the action log - 2026-07-09):** all six Wave 3
  stories are merged to `main` and green (API 907 + web 510 tests): `accounts-identity/08` (kid seat
  presets, #236), `accounts-identity/09` (family device link + the teen-plus adult-signal gate - the
  child-safety seam, merged on owner sign-off: teen-plus content is served ONLY on a server-resolved
  adult signal and is family-safe by default, un-spoofable by a raw hub call, #241), `keepsake-vault/03`
  (vault claim + account-free recovery codes, #240), `keepsake-vault/04` (soft-delete + restore for
  vault tales and moderation takedowns, #237), `control-plane/03` (seven hardcoded knobs migrated onto
  runtime settings keys, #239), and `sysadmin-console/06` (the durable operator action log -
  money/moderation log-before-act with a compiled retention floor, merged on owner sign-off, #238). The
  action log makes `control-plane/01`'s interim `IOperatorActionLog` seam durable. Two tracked
  follow-ups noted: the DEFERRED solo-play teen-plus gate (`accounts-identity/09` AC-07 is
  group-play-scoped) and the `control-plane/01` settings call site's log-after-act ordering. Next: Wave
  4 (`sysadmin-console/07` - support lookup + verbs).
- **Shipped (Wave 2, capabilities + billing metadata - 2026-07-09):** all six Wave 2 stories are
  merged to `main` and green (API 712 + web 460 tests): `accounts-identity/06` (ADR 0002 Decision F
  finally wired - purchaser proof resolved to capabilities at `CreateRoom`, identity discarded at the
  boundary, #225), `accounts-identity/07` (free family account, decoupled from purchase, #220),
  `keepsake-vault/02` (the device gallery becomes a view over the vault, #222), `control-plane/02`
  (system-scope capability flags / kill switches in the entitlement evaluation, #223),
  `sysadmin-console/05` (the jobs shell - Support / Content / Operations - + scoped authz, #221), and
  `billing-entitlements/08` (grant metadata + mode-safe operator Stripe resync - the money seam, #226).
  Layers 0-1 are now complete, so Stripe live is unblocked (still a deliberate, separate go-live).
  Next: Wave 3 (kid seat presets, family device link + teen-plus gate, vault claim/recovery + soft
  delete, knob migration, operator action log).
- **Shipped (Wave 1, the foundation - 2026-07-09):** all five Wave 1 stories are merged to
  `main` and green: `accounts-identity/05` (stable AccountId spine, #201), `keepsake-vault/01`
  (server-side auto-save vault, #204), `control-plane/01` (runtime settings service, #206),
  `sysadmin-console/04` (one console / one auth - the Stripe toggle behind the real Operator
  policy, interim shared-secret gate deleted, #205), and `platform-devops/08` (durable Data
  Protection key ring + CSPRNG signing key + shared magic-link nonce store, #207). One deploy-time
  action carries forward (`accounts-identity/05` AC-05): the pre-ADR-0003 rows in the qa/UAT
  account tables must be cleared once after deploy per
  `docs/runbooks/reset-account-tables-for-account-id-spine.md` (has a "do not run against beta"
  guard). Next: Wave 2 (capabilities + billing metadata) - `accounts/05` is the stable base its
  `control-plane/02` + `billing-entitlements/08` re-key build on.
