...
/// <summary>
/// The real per-anonymous-session AI quota (story 03). Enforces a fixed per-session
/// allowance (<see cref="AiOptions.QuotaPerSession"/>) keyed on the anonymous
/// Room.InstanceId, server-side and before the transport, and reports the remaining
/// count for the client meter. In-memory, thread-safe, and fail-safe (deny on any
/// error). Registered as a singleton in Program.cs, replacing
/// <see cref="UnlimitedAiQuota"/>.
/// </summary>
public sealed class AiQuota : IAiQuota
{
    ...
    // The per-session allowance N, captured once from config (AiOptions.QuotaPerSession).
    // A single source for N (no scattered literals). Clamped at >= 0: a misconfigured
    // negative never becomes "unlimited" - it means "no allowance" (the fail-safe side).
    private readonly int _perSessionLimit;

    private readonly ILogger<AiQuota> _logger;

    public AiQuota(AiOptions options, ILogger<AiQuota> logger)
    {
        // A null options is not expected (DI supplies the registered singleton), but
        // guard defensively so construction never throws - and default to the safe
        // side (0 = no allowance) rather than an unbounded value.
        _perSessionLimit = Math.Max(0, options?.QuotaPerSession ?? 0);
        _logger = logger;
    }
    ...
    public AiQuotaDecision TryConsume(string instanceId)
    {
        try
        {
            ...
            lock (_gate)
            {
                // First call this session starts at the full allowance. A session at
                // or below zero is out - pin it at zero and deny (a clean deny, never
                // a throw, so the caller degrades to the deterministic fallback).
                if (!_remaining.TryGetValue(instanceId, out var left))
                {
                    left = _perSessionLimit;
                }
